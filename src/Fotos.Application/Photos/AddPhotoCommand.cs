...
public sealed record AddPhotoCommand(Guid FolderId, Guid AlbumId, Stream Photo, string ContentType, string FileName)
    : ICommand<Guid>
{
    internal sealed class Handler : ICommandHandler<AddPhotoCommand, Guid>
    {
        ...
        async Task<Result<Guid>> ICommandHandler<AddPhotoCommand, Guid>.Handle(AddPhotoCommand command, CancellationToken cancellationToken)
        {
            var id = Guid.NewGuid();
