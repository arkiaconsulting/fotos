...
namespace Fotos.Adapters.DataStore;
...
internal sealed class AzureCosmosDb
{
    ...
    public async Task StoreSessionData(Guid userId, SessionData sessionData)
    {
        ...
        var cosmosSessionData = new CosmosSessionData(userId, [.. sessionData.FolderStack.Select(x => new CosmosFolder(x.Id, x.ParentId, x.Name.Value)).Reverse()]);
    ...
    public async Task<SessionData?> GetSessionData(Guid userId)
    {
        ...
        try
        {
            ...
            return new SessionData(new Stack<Folder>(response.Resource.FolderStack.Select(x => new Folder { Id = x.Id, ParentId = x.ParentId, Name = Name.Create(x.Name) })));
        }
